...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      width = 8,
      img(src = "dataset-cover.jpg", height = 140, width = 1050),
      uiOutput("space"),
      conditionalPanel(
        ...
        conditionalPanel(
          ...
          leafletOutput("map", width = "101.4%", height = "300px")
        ),
    ...
    if (!is.na(selected) && !is.null(selected)) {
      if (input$topics == "Circuits") {
        ...
        leaflet() %>%
          ...
          addTiles() %>%
          addMarkers(lng = test$lng, lat = test$lat, label = paste(test$circuitRef, test$location, sep = ", "))
      }
    }
  })
